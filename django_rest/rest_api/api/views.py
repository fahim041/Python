...
def release_details(request, pk):
    try:
        release = Release.objects.get(city=pk)
        #article = Article.objects.filter(release=pk)
    ...
    if request.method == 'GET':
        ...
        article = Article.objects.filter(release=serializer.data['id'])
        articleData = ArticleSerializer(article, many=True)
